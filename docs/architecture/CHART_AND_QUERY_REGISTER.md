...
| `horse-in-stall-seven-day` | Horse Detail → Summary → Stall | What is this horse's pattern of being in and out of its stall over the last seven days? | **Decided (Inakshi, 2026-09-05):** one strip per barn day, today at the TOP, three states with no legend — in (denim), out (pale), not recorded (grey) — and the rest of today dashed. Tap a row for the Horse in Stall wording ("Tuesday · 17 h 50 min · Out 8:30 AM – 12:10 PM"). **Occupancy only**: the dev team's HC84-36095 tints cells by activeness and needed a legend line to explain it; "if a chart needs a sentence explaining what the colour means, the colour shouldn't be there — it's not a dashboard." Activeness keeps its own chart in Horse Trends | **meaning-blocked**, as `horse-in-stall-daily`. Plus **CQ-8**: every row is from the CURRENT stall until the backend supplies dated assignment history; the chart says so under the rows and must not go live before that lands | Fixture-backed behind `PREVIEWS.horseInStallSampleData`, mounted in the Stall card; **no live path** | Canonical seconds; displayed h/min | As `horse-in-stall-daily` | Pending | **meaning-blocked** |
| `horse-lying-down-timeline` | Horse Detail → Summary (and Stall Details, shared) | When, over the last 7 days, was this horse lying down — and where do we not know? | **Decided (Inakshi, 2026-09-07, mockup v3):** the ORIGINAL chart — bars on plain day rows, oldest first, tap for times + duration — plus exactly one addition: a dashed "No readings" stretch. No "up" track, no "before this stall" state, no sentences. Midnight in the org zone | Query is the app's `horse_sitting_per_id{animal_type="horse"}`; sample meaning from metadata (90 s window behind T − 1 min); union opt-in; bout ends at last positive sample. **Identity and assignment-history remain named blockers** — see spec §5 | Fixture-backed behind `PREVIEWS.lyingDownTimelineSampleData`; **no live path** | Seconds; tooltip h/min, sub-minute in seconds | All 8 brief §4 cases as tests, expected values stated before running | [`chart-specs/horse-lying-down-timeline.md`](chart-specs/horse-lying-down-timeline.md) | **building** |
...
1. Absent reading must never render as a reassuring verdict. CQ-6 closed
   2026-09-04: the three live score cards now judge freshness by the sample's
   own timestamp as well as the phone's receipt time, so an old reading says
   `Updated N min ago` and expires after fifteen minutes even online.
...
5. Multiple matching series must not be blindly summed (a day cannot exceed 24
   hours); the combination rule needs Data Science sign-off. CQ-9 closed
   2026-09-04: the three live score cards fail closed on duplicate same-name
   series (Activeness already did; Temperature and Noise now share the rule).
...
3. **People In Stall ignores head-count changes when counting visits.** Its
   approved measure is occupied time, so `1 → 2 → 1` remains one visit.
   Count-based occupancy charts still retain value changes. Closed 2026-09-04.
...
The shared honest-state defects CQ-3, CQ-4 and CQ-5 were closed 2026-09-04.
For CQ-8, historical score queries are now withheld rather than run against the
horse's current stall; the final solution remains blocked on effective-dated
assignment data from the backend.
