...
| **B3** | **Global "Show Me" search** — **DECIDED 2026-08-17: RETIRED.** Per-page search (Horses header search bar) is the replacement; no cross-entity search screen in the rewrite | The header magnifier opened a cross-entity search over **horses, stalls and behaviours** at once. | The rewrite's header search filters *horses on this page only*. Better for finding a horse; the cross-entity entry point is gone. Decide whether Show Me returns elsewhere, or is retired. |
...
| **E4** | **Illustrated states** — **DECIDED 2026-08-17: keep the small glyph, no artwork** | Per-page artwork: `empty-horses`, `error-horses`, `error-no-internet`, `empty-search`. | A small glyph in a grey circle. | Deliberate under the editorial palette — **recorded so it is not "fixed" back**. The artwork exists and is unused; decide once. |
