...
2. **Clock time projected onto a fixed 1970 date.** `normalizeToBaseDate` maps every timestamp to `1970-01-05 hh:mm:ss`, so on a daylight-saving day (23 or 25 h) bars after the change sit up to an hour off. → Position = seconds since *this* day's real midnight over *this* day's real length. Test: *"keeps clock time honest on the 23-hour spring-forward day"*; fixture `daylight-saving`.
