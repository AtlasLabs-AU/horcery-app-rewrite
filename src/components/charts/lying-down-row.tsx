...
const BADGE: Record<Verdict, { label: string; bg: string; fg: string }> = {
  usual: { label: 'Usual', bg: COLOURS.okBg, fg: COLOURS.ok },
  low: { label: 'Low', bg: COLOURS.warnBg, fg: COLOURS.warn },
  high: { label: 'High', bg: COLOURS.warnBg, fg: COLOURS.warn },
  'no-data': { label: 'No data', bg: COLOURS.track, fg: COLOURS.muted },
  unknown: { label: 'Not enough history', bg: COLOURS.track, fg: COLOURS.muted },
};
  ...
  return (
    <View style={[styles.row, { width }]}>
      ...
      {day ? (
        <>
            ...
            {usualCurve && usualCurve.length > 1
                  ...
                  return (
                    <View
                      ...
                      style={{
                        ...
                        top,
                        height,
                        backgroundColor: COLOURS.ribbon,
                      }}
                    />
                  );
                })
              ...
              <View
                style={{
                  ...
                  height: 1,
                  backgroundColor: COLOURS.ribbonEdge,
                }}
              />
            ) : null}
            <CumulativeLine week={week} day={day} width={chartWidth} peakSeconds={ceiling} />
          </View>

          {/* in-stall strip — when the horse was observable at all */}
          <View style={[styles.strip, { width: chartWidth }]}>
          ...
          <View style={[styles.axis, { width: chartWidth }]}>
            ...
            {axisLabels(week.dayStartHour).map((label, index) => (
              <Text key={`${index}-${label}`} style={styles.axisText}>
                {label}
              </Text>
            ))}
          </View>

          <Text style={styles.stallSummary}>
        ...
        <Text style={styles.stallSummary}>No reading for today</Text>
      )}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 14, fontWeight: '600', color: COLOURS.ink, flexShrink: 1 },
  badge: { paddingHorizontal: 7, paddingVertical: 2, borderRadius: 99 },
  badgeText: { fontSize: 10, fontWeight: '600' },
  figures: { alignItems: 'flex-end' },
  today: { fontSize: 14, fontWeight: '700', color: COLOURS.ink },
  average: { fontSize: 10, color: COLOURS.muted, marginTop: 1 },
  chart: { marginTop: 8, backgroundColor: COLOURS.track, borderRadius: 6, overflow: 'hidden' },
  strip: { height: STRIP_HEIGHT, marginTop: 3, backgroundColor: COLOURS.track, borderRadius: 3 },
  axis: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 3 },
  axisText: { fontSize: 9, color: COLOURS.muted },
  stallSummary: { fontSize: 10.5, color: COLOURS.muted, marginTop: 5 },
});
